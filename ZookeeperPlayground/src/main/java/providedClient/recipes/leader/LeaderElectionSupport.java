 ...
 */
package providedClient.recipes.leader;
 ...
 * instance of {@link org.apache.zookeeper.ZooKeeper} or the proper connection information. Once
...
public class LeaderElectionSupport implements Watcher {
  ...
  private void determineElectionStatus() throws KeeperException,
    ...
    // !! very important to set the watch to false
  ...
  // register neighbour's deleted event
  public void process(WatchedEvent event) {
    if (event.getType().equals(Event.EventType.NodeDeleted)) {
      if (!event.getPath().equals(leaderOffer.getNodePath()) && state != State.STOP) {
        ...
        try {
          // re-ensure that neighbour node is deleted.
          determineElectionStatus();
        } catch (KeeperException e) {
          becomeFailed(e);
        } catch (InterruptedException e) {
